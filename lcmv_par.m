function [A, A1, po]=lcmv_par(L,C,alpha)
...
parfor i=1:ns
    Lloc=squeeze(L(:,i,:));
   A(:,i,:)=reshape((pinv((Lloc'*Crinv*Lloc))*Lloc'*Crinv)',nchan,ndim); %#ok<*MINV>
   %  A(:,i,:)=reshape(Crinv*Lloc*inv(Lloc'*Crinv*Lloc),nchan,ndim); %#ok<*MINV>
